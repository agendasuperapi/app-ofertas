-- Atualizar função get_affiliate_stores para calcular comissões baseadas no status do PEDIDO
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(
      CASE 
        WHEN o.status NOT IN ('cancelado', 'cancelled') 
        THEN ae.order_total 
        ELSE 0 
      END
    ...
    COALESCE(SUM(
      CASE 
        WHEN o.status IN ('entregue', 'delivered') 
    ...
    COALESCE(SUM(
      CASE 
        WHEN o.status NOT IN ('entregue', 'delivered', 'cancelado', 'cancelled') 
        THEN ae.commission_amount 
        ELSE 0 
      END
...
-- Atualizar função get_affiliate_consolidated_stats para calcular baseado no status do PEDIDO
...
BEGIN
  ...
  SELECT 
    ...
    -- Pago (alias para total_commission - mantido por compatibilidade)
    COALESCE(SUM(
      CASE 
        WHEN o.status IN ('entregue', 'delivered') 
        THEN ae.commission_amount 
