...
export default function AffiliateDashboardNew() {
  ...
  const renderStoreModalContent = (store: typeof affiliateStores[0]) => <Tabs defaultValue="overview" className="mt-4 flex-1 flex flex-col min-h-0 px-4">
      ...
      <TabsContent value="history" className="space-y-4 mt-4 flex-1 overflow-y-auto">
        {(() => {
          // Buscar todas as transações: ganhos (pedidos entregues) e saques pagos
          const storeOrders = affiliateOrders
            .filter(order => order.store_id === store.store_id)
            .filter(order => order.order_status === 'entregue' || order.order_status === 'delivered');
          
          const storeWithdrawals = withdrawalRequests
            .filter(req => req.store_id === store.store_id && req.status === 'paid');
          ...
          // Adicionar ganhos (créditos)
          storeOrders.forEach(order => {
            transactions.push({
              id: `order-${order.earning_id}`,
              date: order.order_date,
              type: 'credit',
              description: `Comissão pedido #${order.order_number}`,
          ...
          // Adicionar saques (débitos)
          storeWithdrawals.forEach(withdrawal => {
            transactions.push({
          ...
          // Calcular saldo corrente (do mais antigo ao mais recente para saldo)
                ...
                <div className="space-y-1">
                  ...
                  {paginatedTransactions.map((transaction, index) => (
                    <motion.div
                      ...
                      className={`grid grid-cols-12 gap-2 px-3 py-2.5 rounded-lg border transition-colors ${
